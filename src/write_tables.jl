...
function write_tables(R :: Result, filename :: String)
  ...
  println(output,@sprintf("#"))
  println(output,@sprintf("# Density of solvent in simulation box (sites/A^3): %12.5f",R.density.solvent))
  println(output,@sprintf("# Density of solvent in bulk (estimated) (sites/A^3): %12.5f",R.density.solvent_bulk))
  println(output,@sprintf("# Molar volume of solvent in simulation (cc/mol): %12.5f",convert/R.density.solvent))
  println(output,@sprintf("# Molar volume of solvent in bulk (estimated) (cc/mol): %12.5f",convert/R.density.solvent_bulk))
  println(output,@sprintf("#"))
  println(output,@sprintf("# Number of atoms solute: %i7",size(R.solute_atom,2)))
  println(output,@sprintf("# Number of atoms of the solvent: %i7",size(R.solvent_atom,2)))
  println(output,@sprintf("#"))
  ...
  for i in 1:size(R.solvent_atom,2)
    println(output,@sprintf("# Atom of type: %6i",i))
  end
  println(output,"#")
  string = "#     DISTANCE     GMD TOTAL" 
  for i in 1:size(R.solvent_atom,2)
    string = string*@sprintf("  %i12",i)
  end
  ...
  println(output,"#")
  println(output,"# Trajectory file: ", R.files)
  println(output,"#")
  println(output,"# Atoms:")
  for i in 1:size(R.solute_atom,2)
    println(output,@sprintf("#%6i",i))
  end
  println(output,"#")
  string = "#     DISTANCE      GMD TOTAL"
  for i in 1:size(R.solute_atom,2)
