...
distance(x :: Array{Float64}, y :: Array{Float64}, j :: Int64) = 
  sqrt((y[j,1]-x[1])^2 + (y[j,2]-x[2])^2 + (y[j,3]-x[3])^2)

distance(x :: Array{Float64}, y :: Array{Float64}, i :: Int64, j :: Int64) = 
  sqrt((y[j,1]-x[i,1])^2 + (y[j,2]-x[i,2])^2 + (y[j,3]-x[i,3])^2)

# With possible periodic conditions 

function distance( box :: Box, x :: Array{Float64}, y :: Array{Float64}, 
                               i :: Int64, j :: Int64 )
