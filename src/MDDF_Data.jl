...
function MDDF_Data( solute :: Solute, 
  ...
  if irefatom > solvent.natoms 
    error("in MDDF input: Reference atom index", irefatom, " is greater than number of "*
          "                      atoms of the solvent molecule. ")
  end

  # The number of random samples for numerical normalization
  if n_random_samples == -1
    n_random_samples = max(10*solvent.nmols,solute.nmols)
  end
  if n_random_samples < solute.nmols
  ...
  return MDDF_Data( nbins = nbins, # number of bins of histogram
                    ...
                    density = Density(),
                    volume = Volume()
                    OutputFiles( output_name, # name of main output file
